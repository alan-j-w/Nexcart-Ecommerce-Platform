      ...
      <div style={{ display: "flex", gap: "40px", background: "#fff", borderRadius: "4px", padding: "30px" }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: "13px", color: "#6B7280", marginBottom: "16px" }}>
            M.R.P.: <span style={{ textDecoration: "line-through" }}>₹{originalPrice.toLocaleString("en-IN")}</span> Inclusive of all taxes
          </p>
        ...
        {/* Right — Buy Box */}
        <div style={{ width: "260px", flexShrink: 0, border: "1px solid #E5E7EB", borderRadius: "8px", padding: "20px", height: "fit-content" }}>
          ...
          {product.stock > 0 ? (
            <p style={{ fontSize: "16px", color: "#059669", fontWeight: 600, marginBottom: "12px" }}>In Stock</p>
          ) : (
            <p style={{ fontSize: "16px", color: "#DC2626", fontWeight: 600, marginBottom: "12px" }}>Out of Stock</p>
          )}
          ...
          {product.stock > 0 && (
            <>
              ...
              <button className="mm-btn-buy" onClick={() => addToCart().then(() => router.push("/cart"))}>
                Buy Now
              </button>
            </>
          )}
          ...
          {message && (
            <p style={{ fontSize: "13px", color: message.includes("✓") ? "#059669" : "#DC2626", marginTop: "10px", textAlign: "center", fontWeight: 600 }}>
              {message}
            </p>
          )}

          <hr style={{ margin: "14px 0", border: "none", borderTop: "1px solid #E5E7EB" }} />
          <p style={{ fontSize: "12px", color: "#6B7280" }}><strong>Sold by:</strong> {vendorName}</p>
          <p style={{ fontSize: "12px", color: "#6B7280" }}><strong>Fulfilled by:</strong> Nexcart</p>
        </div>
      </div>
    </div>
  );
